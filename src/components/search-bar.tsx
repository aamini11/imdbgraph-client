...
export function SearchBar() {
  ...
  return (
    <search className="w-full">
      <Command
        className={cn(
          "bg-background text-popover-foreground placeholder:text-muted-foreground flex h-full w-full flex-col rounded-md",
      ...
      >
        ...
        <div className="border-input flex items-center rounded-full border px-3">
          ...
          <CommandInput
            ...
            className={cn(
              "placeholder:text-muted-foreground h-10 w-full text-sm outline-none disabled:cursor-not-allowed disabled:opacity-50",
            )}
        ...
        {/* Dropdown Menu */}
        <CommandList
          className={cn("mt-2 w-full rounded-xl border p-2", {
            hidden: !value,
          })}
          ...
          {showLoading ? (
            ...
            searchResults.map((show) => (
              <CommandItem
                ...
                key={show.imdbId}
                className={cn(
                  "data-[selected=true]:bg-accent data-[selected=true]:text-accent-foreground flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
                  "text-foreground/60 px-2 py-1.5 text-sm",
